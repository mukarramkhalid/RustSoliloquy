...
unsafe impl GlobalAlloc for NtVirtualAlloc {
    ...
    /// The memory is allocated with `PAGE_READWRITE` protection.
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        run_syscall!(
            ntdll().nt_allocate_virtual_memory.number,
            ntdll().nt_allocate_virtual_memory.address as usize,
            h_process,
            &mut p_address,
            0,
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        run_syscall!(
            ntdll().nt_free_virtual_memory.number,
            ntdll().nt_free_virtual_memory.address as usize,
            h_process,
